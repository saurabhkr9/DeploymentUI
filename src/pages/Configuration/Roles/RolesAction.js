...
export const RolesAction = (props) => {
    ...
    const [IBMCloude, setIBMCloude] = useState("No");
    const [createDate, setCreateDate] =useState(new Date());
    const [updateDate, setUpdateDate] =useState(new Date());

    useEffect(() => {
        setRoleName(selectedRow.role_name);
        setCreatedBy(selectedRow.created_by); 
        setUpdatedBy(selectedRow.updated_by); 
        setIBMCloude(selectedRow.access === undefined ? "No" : "Yes")
    ...
    const section = {
        accessHubModal: {
          ...
          modalTitlePrimary: 'Role\'s Information',
          ...
          <div className="bx--row details-form-row">
              ...
              <TextInput
                  ...
                  defaultValue={selectedRow.role_name ?? ""}
          ...
          modalAction:  async () => {
            const data = {
              ...
              if(selectedRow.id !== undefined){
                  await dispatch(updateAction(data,selectedRow.id, props.url, projectAction.updateRoles ))
            }else{
                  await dispatch(addNewAction(data, props.url, projectAction.addNewRole, 'Role inserted to db successfully!' ))
              }
             setRoleName(""); 
            }
        }
